...
from genki_signals.signal_sources import ManualTimerSignalSource, MouseSignalSource
...
def main(host, port, sampling_rate, fetch_x_sec, debug):
    source = ManualTimerSignalSource(
        sampling_rate=sampling_rate, secondary_sources=[MouseSignalSource()]
    )
    global system
    system = SignalSystem(
