...
deserialized without any issues, and their parameters can be treated
like hyperparameters in a machine learning model. When a data session
is recorded with a signal system, only the json serialisation of
the signal functions is stored. Signal functions are also assumed
to be causal, i.e. they only depend on past values of the input
and can thus be computed in real time.
"""
