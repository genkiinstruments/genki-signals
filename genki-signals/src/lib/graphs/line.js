...
export function create_line(
	...
	const xScale = d3.scaleLinear().domain([x_domain.min, x_domain.max]).range([0, svg_width]);

	const yScale = d3.scaleLinear().domain([y_domain.min, y_domain.max]).range([0, svg_height]);
	...
	for (let i = 0; i < sig_y.length; i++) {
		...
		let path = pathG.append('path').datum(data_buffer.view()).attr('class', 'line');
