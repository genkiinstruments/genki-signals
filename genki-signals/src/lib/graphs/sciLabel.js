...
import {DataPointSelectionModifier, ESelectionMode} from "scichart/Charting/ChartModifiers/DataPointSelectionModifier";
...
class LinePaletteProvider extends DefaultPaletteProvider {
    constructor(options) {
        ...
        if (options === null || options === void 0 ? void 0 : options.selectedLineStroke) {
            this.selectedLineStroke = (0, parseColorToUIntArgb)(options === null || options === void 0 ? void 0 : options.selectedLineStroke);
        }
        if (options === null || options === void 0 ? void 0 : options.selectedPointStroke) {
            this.selectedPointStroke = (0, parseColorToUIntArgb)(options === null || options === void 0 ? void 0 : options.selectedPointStroke);
        }
        if (options === null || options === void 0 ? void 0 : options.selectedPointFill) {
            this.selectedPointFill = (0, parseColorToUIntArgb)(options === null || options === void 0 ? void 0 : options.selectedPointFill);
        }
...
export async function initSciLabel(divname) {
    ...
    for(let i = 0; i < row_count*col_count - 2; i++){
        ...
        const xyDataSeries = new XyDataSeries(wasmContext, { xValues, yValues, dataSeriesName: "Series_"+i, metadata: timestamps.map((d) => ({"timestamp": d, isSelected: false}))});
        ...
        subChartSurface.renderableSeries.add(new FastLineRenderableSeries(wasmContext, {
            id: "Series"+i,
        ...
        dataPointSelection.selectionChanged.subscribe((data) => {
            ...
            /** @type {number[] | undefined} */ let currSelected = data?.selectedDataPoints.map((d) => (d.index))
            for(let series of dataSeries){
                for(let i of lastSelected){series.getMetadataAt(i).isSelected = false}
                for(let i of currSelected){series.getMetadataAt(i).isSelected = true}
            }
        ...
        subChartSurface.chartModifiers.add(new RolloverModifier({modifierGroup: "cursorGroup", showTooltip: false}))
